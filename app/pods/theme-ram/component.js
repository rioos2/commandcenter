...
export default Ember.Component.extend({
  ...
  ram: function() {
    ...
    this.set("model.assemblyfactory.properties.ram", ram);
  }.observes('ramData'),
  ...
  renderChartRam: function(self, params) {
    ...
    var main = function(selection) {
      selection.each(function scope() {
        ...
        calc.pieCenterY = -calc.chartHeight * 0.2;
        ...
        var image = patternify({
          container: svg,
          selector: 'svg-background-image',
          elementTag: 'svg:image'
        })
        image
          .attr('width', attrs.svgWidth)
          .attr('height', attrs.svgHeight)
          .attr("xlink:href", "images/second/ram.png")
        ...
        var clipPath = patternify({
            container: svg,
            selector: 'svg-defs-clip-path',
            elementTag: 'clipPath'
          })
          .attr('id', 'my-clip')

        var clipPathRect = patternify({
          container: clipPath,
          selector: 'svg-defs-clip-path-rect',
          elementTag: 'rect'
        })
        ...
        // self.set('ramData', attrs.data.value);
        //pieCenterPoint
        var pieCenterPoint = patternify({
          container: chart,
          selector: 'pie-center-point',
          elementTag: 'g'
        })
        pieCenterPoint.attr('transform', 'translate(' + (calc.pieCenterX) + ',' + calc.pieCenterY + ')');

        //passive progress paths
        var piePaths = patternify({
          container: pieCenterPoint,
          selector: 'pie-paths',
          elementTag: 'path',
          data: layouts.pie(calc.progressProps.top)
        })
        ...
        var piePaths = patternify({
          container: pieCenterPoint,
          selector: 'active-pie-paths',
          elementTag: 'path',
          data: layouts.pie(calc.progressProps.topActive)
        })
        self.set('ramData', attrs.data.value);
        ...
        piePaths.attr('d', arcs.top)
          ...
          .each(d => {
            if (d.data.mainNode) {
              calc.minX = arcs.top.centroid({
                startAngle: d.endAngle,
                endAngle: d.endAngle
              })[0];
              calc.maxX = -calc.minX;
              scales.slider.range([calc.minX, calc.maxX]);
              sliderButtonPosition = arcs.top.centroid({
        ...
        var sliderCircleButton = patternify({
          container: pieCenterPoint,
          selector: 'slider-circle-button',
          elementTag: 'circle'
        })
        ...
        var bottomPasivePaths = patternify({
          container: pieCenterPoint,
          selector: 'bottom-passive-paths',
          elementTag: 'path',
          data: layouts.pie(calc.progressProps.bottom)
        })
        ...
        var bottomActivePaths = patternify({
          container: pieCenterPoint,
          selector: 'dashs-paths',
          elementTag: 'path'
        })
