...
export default Component.extend({
    ...
    reportNodesUpdate: function() {
      if(!Ember.isEmpty(this.get('counterModel.content'))) {
        this.get('counterModel.content').objectAt(0).results.statistics.nodes.forEach((mn) => {
         if(this.get('model.id') == mn.id){
           this.set('model.counter', mn.counter);
         }
        });
      }
    }.observes('counterModel.content.@each.results.statistics.nodes.@each.counter', 'model'),
});
